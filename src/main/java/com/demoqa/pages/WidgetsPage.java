...
public class WidgetsPage extends BasePage{
    ...
    @FindBy(id = "cars")
    WebElement cars;
    public WidgetsPage standardMultiSelect(String[] carBrands) {
        Select select = new Select(cars);
        for (int i = 0; i < carBrands.length; i++) {
            if (select.isMultiple()) {
                select.selectByVisibleText(carBrands[i]);
            }
        }
    ...
    public WidgetsPage moveSlider() {
        ...
        actions.dragAndDropBy(sliderContainer,255, 0).perform();
        pause(1000);
        return this;
    }
    ...
    public WidgetsPage verifySubMenu() {
        Assert.assertTrue(isElementVisible(subSubItem1));
        return this;
    }
    ...
    public WidgetsPage autoComplete() {
        moveWithJS(autoCompleteMultipleInput, 0, 300);
